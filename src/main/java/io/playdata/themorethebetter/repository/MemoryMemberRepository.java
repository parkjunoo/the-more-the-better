...
public class MemoryMemberRepository implements MemberRepository
{
	...
	{
		member.setMem_no(++sequence); // 아이디 값을 세팅해 줌
		store.put(member.getMem_no(), member);
	...
	{
		return store.values().stream().filter(member -> member.getMem_name().equals(mem_name)).findAny(); // findAny() : 하나라도 찾는 것
	}

	@Override
	public Optional<Member> findById(String mem_id) 
	{
		return store.values().stream().filter(member -> member.getMem_id().equals(mem_id)).findAny();
	}
